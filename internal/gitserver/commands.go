...
func (opt CommitsOptions) ToProto() (*proto.CommitLogRequest, error) {
	...
	if opt.AllRefs {
		p.Commits = &proto.CommitLogRequest_AllRefs{
			AllRefs: true,
		}
	} else if opt.Range != "" {
		p.Commits = &proto.CommitLogRequest_Range{
			Range: []byte(opt.Range),
		}
	}

	if !opt.After.IsZero() {
		p.After = timestamppb.New(opt.After)
	}
	if !opt.Before.IsZero() {
		p.Before = timestamppb.New(opt.Before)
	}
